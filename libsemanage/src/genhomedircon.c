				      ...
				      semanage_list_t * homedir_context_tpl)
{
	...
	for (; users; pop_user_entry(&users)) {
		// Don't write contexts for users mapped to
		// fallback logins since they are already covered
		// by the fallback regexps.
		if (strcmp(users->login, DEFAULT_LOGIN) && strcmp(users->sename,  s->fallback->sename) == 0)
			continue;
